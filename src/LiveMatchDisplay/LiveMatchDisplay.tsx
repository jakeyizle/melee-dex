...
import { selectAllReplayNames, Replay, selectReplaysWithBothPlayers } from "@/db/replays"
...
export const LiveMatchDisplay = () => {
  ...
  const [currentReplayInfo, setCurrentReplayInfo] = useState<CurrentReplayInfo | null>(null);
  ...
  useEffect(() => {
    const fetchSettings = async () => {
        ...
        if (!!replayDirectory) {
          ...
          window.ipcRenderer.invoke('listen-for-new-replays', { replayDirectory });
        }
    }
    fetchSettings();
...
useEffect(() => {
    const newReplayLoaded = async (_event: any, args: { filename: string, players: LiveReplayPlayers[], stageId: string }) => {
      const { filename, players, stageId } = args;      
      if (filename === currentFileName?.current) return;
      const historicalReplays = await selectReplaysWithBothPlayers([players[0].connectCode, players[1].connectCode]);
      ...
      <Stack spacing={3}>
        ...
        <Grid container spacing={3}>
          <Grid size={{xs: 12, md: 6}}>
            <CurrentMatchCard players={currentReplayInfo.players} stageId={currentReplayInfo.stageId} />
          </Grid>

          <Grid size={{xs: 12, md: 6}}>
            <PlayerStatsCard players={currentReplayInfo.players} historicalReplays={historicalReplays} />
          </Grid>
        </Grid>
) :
(
